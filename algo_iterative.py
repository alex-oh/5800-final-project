# -*- coding: utf-8 -*-
"""Algo_Iterative 2.0.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1R1VBoQupiK9hmWkavqwxW5ltWtJI_wGC
"""
...
result_df = pd.DataFrame(columns=['Transactions', 'Edges', 'TimeTaken'])

# Function that processes each row's 'Transactions' column
def process_transactions(row):
    transactions = eval(row['Transactions'])  # Assuming the data in the 'Transactions' column is stored as a string representation of a list
    print("Processing transactions:", transactions)
    start_time = time.time()  # Record start time
    edges = calculateMinCashFlow(transactions)  # Call your existing function here and get the calculated edges
    end_time = time.time()  # Record end time
    time_taken = end_time - start_time  # Calculate time taken

    # Append the processed information to the result DataFrame
    result_df.loc[len(result_df)] = [str(transactions), edges, time_taken]

    print(f"Time taken for processing: {time_taken} seconds")
    print("-------------------------------------------------------------------------------------------")


# Loop through the DataFrame rows and process the 'Transactions' column
for index, row in df.iterrows():
    process_transactions(row)

# Display the resulting DataFrame with processed information
print("Result DataFrame:")
result_df

df_iterative = result_df.copy()  # Make a copy of the DataFrame with the new name

# Write the DataFrame to a CSV file
result_df.to_csv('df_iterative.csv', index=False)

from google.colab import files

files.download('df_iterative.csv')